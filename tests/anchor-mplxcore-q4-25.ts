...
describe("anchor-mplxcore-q4-25", () => {
  ...
  before(async () => {
    ...
    // Debug: print upgrade authority
    if (programData && programData.data.length >= 45) {
      const hasAuthority = programData.data[8];
      console.log(`ProgramData length: ${programData.data.length}, hasAuthority flag: ${hasAuthority}`);
      if (hasAuthority === 1) {
        const authorityBytes = programData.data.slice(9, 41);
        ...
        console.log(`>>> Authority matches payer: ${upgradeAuthority.equals(payer.publicKey)}`);
      }
    } else {
      console.log(`ProgramData is null or too short`);
    }
